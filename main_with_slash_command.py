...
bad_words = [
    'fuck', 'shit', 'bitch', 'ass', 'pussy', 'dick', 'cunt', 'pussy', 'asshole',
    'assholes', 'bitch', 'fucker', 'fuckers', 'fucking', 'fucks', 'fucked', 'fucked',
    # Ukrainian bad words
    'блядь', 'хуй', 'пизда', 'сука', 'шлюха', 'дєбіл', 'ідіот', 'гандон',
    'сучара', 'мудак', 'заєбал', 'срака', 'підарас', 'хуйло', 'піздєц', 
    'їбати', 'курва', 'пиздюк', 'хер', 'їбало', 'залупа', 'манда', 'проститутка',
    'довбойоб', 'нахуй', 'єбут', 'олень', 'тупий', 'підар', 'лох',
    'розпиздяй', 'засранець', 'гівно', 'гнида', 'чмо',
    # Russian bad words
    'блять', 'сука', 'пизда', 'хуй', 'ебать', 'ёб твою мать', 'шлюха', 'мудак',
    'идиот', 'уебок', 'гандон', 'заебал', 'долбаёб', 'пидр', 'ублюдок', 'жопа',
    'хуесос', 'выебон', 'еблан', 'пидарас', 'манда', 'залупа', 'хер с ним',
    'ебало', 'пиздюк', 'нахуй', 'охуел', 'наебениться', 'блядун', 'заебись',
    'чмо', 'говно', 'дерьмо', 'очко', 'выебываться', 'похуй', 'мразь',
    'проебаться', 'ссанина', 'засранец', 'мудозвон'
]
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        embed = discord.Embed(description="This command does not exist. Please use a valid command.", color=discord.Color.red())
    elif isinstance(error, commands.MissingRequiredArgument):
        embed = discord.Embed(description="Missing a required argument. Please pass all the required arguments.", color=discord.Color.red())
    elif isinstance(error, commands.MissingPermissions):
        embed = discord.Embed(description="You do not have the required permissions to execute this command.", color=discord.Color.red())
    elif isinstance(error, commands.CheckFailure):
        embed = discord.Embed(description="You do not have the required role to execute this command.", color=discord.Color.red())
    elif isinstance(error, commands.CommandOnCooldown):
        embed = discord.Embed(description=f"This command is on cooldown. Please try again after {error.retry_after:.2f}s.", color=discord.Color.red())
    elif isinstance(error, commands.BadArgument):
        embed = discord.Embed(description="Invalid argument. Please provide a correct one.", color=discord.Color.red())
    elif isinstance(error, commands.CommandInvokeError):
        embed = discord.Embed(description="An error occurred while executing the command.", color=discord.Color.red())
    else:
        embed = discord.Embed(description="An unexpected error occurred.", color=discord.Color.red())
...
async def on_message(message):
    ...
    if any(bad_word in msg for bad_word in bad_words):
        ...
        embed = discord.Embed(title="Inappropriate Language", description=f"{message.author.mention} Bad words are not allowed here!", color=discord.Color.red())
...
async def on_member_join(member):
  ...
  if channel is not None and role is not None:
    try:
      ...
      embed = discord.Embed(
          description=
          f'Пользователь ``{member.display_name}``, присоединился к нам!',
          color=0x00ff00)
...
async def card_user(ctx, *, member: discord.Member = None):
    ...
    embed = discord.Embed(title=f"{member}'s User Card", color=discord.Color.blue())
    embed.set_thumbnail(url=member.avatar.url)
    embed.add_field(name="Username", value=member.display_name, inline=True)
    embed.add_field(name="User ID", value=member.id, inline=True)
    embed.add_field(name="Account Created", value=member.created_at.strftime('%B %d, %Y'), inline=True)
    embed.add_field(name="Joined Server", value=member.joined_at.strftime('%B %d, %Y'), inline=True)
    embed.add_field(name="Top Role", value=member.top_role.name, inline=False) # Changed inline to False for better readability
    await ctx.send(embed=embed)
...
async def clear_error(ctx, error):
    error_mapping = {
        commands.MissingRequiredArgument: 'Please specify the number of messages to delete.',
        commands.MissingPermissions: 'You do not have the required permissions.',
        commands.BadArgument: 'Please specify a valid number.',
    }
    error_message = error_mapping.get(type(error), 'An unknown error has occurred.')
